...
휴대폰으로 촬영한 스쿼트 영상을 올리면 MediaPipe Pose Landmarker가
33개 신체 지점을 찾아 반복 횟수, 무릎 각도, 상체 기울기와 좌우 균형을
분석합니다. 이 결과는 운동 학습을 돕기 위한 참고 자료이며 의료 진단이 아닙니다.
"""
...
def frame_metrics(landmarks, width, height, camera_view):
    """한 장면에서 스쿼트 자세를 설명하는 각도와 비율을 계산합니다."""
    ...
    if sum(value >= 0.45 for value in visibility) < 6:
        return None
    ...
    issues = []
    if average_knee < 150 and trunk_lean > 43:
        issues.append(("lean", "상체 기울기"))
    if average_knee < 150 and asymmetry > 22:
        issues.append(("balance", "좌우 불균형"))
    if knee_inward:
        issues.append(("knee", "무릎 정렬"))
...
def draw_frame_information(frame, rep_count, metrics, issues):
    ...
    cv2.putText(
        frame, f"REPS {rep_count}", (16, 34),
        cv2.FONT_HERSHEY_SIMPLEX, scale, (255, 255, 255), 2, cv2.LINE_AA,
    )
    if metrics:
        cv2.putText(
            frame,
            f"KNEE {metrics['average_knee']:.0f} deg",
            (16, 68),
...
def browser_friendly_video(raw_path, output_directory):
    ...
    try:
        ...
        target_path = Path(output_directory) / "squat_analysis_h264.mp4"
        command = [
...
def build_feedback(result, camera_view):
    ...
    if result["reps"] == 0:
        feedback.append(
            "완전한 반복 동작을 찾지 못했습니다. 서 있는 자세에서 시작해 앉았다가 "
            "다시 완전히 일어나는 장면까지 촬영해 주세요."
        )
    if result["minimum_knee_angle"] > 120:
        feedback.append(
            "앉는 깊이가 비교적 얕게 나타났습니다. 통증이 없는 범위에서 엉덩이를 "
            "조금 더 낮춰 보세요."
        )
    if result["lean_issue_rate"] > 15:
        feedback.append(
            "일부 장면에서 상체가 많이 숙여졌습니다. 가슴을 편 상태로 엉덩이를 "
            "뒤로 보내는 느낌을 연습해 보세요."
        )
    if result["balance_issue_rate"] > 15:
        feedback.append(
            "왼쪽과 오른쪽 무릎 각도의 차이가 크게 나타난 장면이 있습니다. "
            "양발에 체중을 고르게 실어 보세요."
        )
    if camera_view == "정면" and result["knee_issue_rate"] > 10:
        feedback.append(
...
def analyze_squat_video(video_bytes, filename, camera_view, progress_callback=None):
...
def make_angle_chart(result):
    """시간에 따른 양쪽 무릎 각도를 휴대폰용 그래프로 표시합니다."""
    import plotly.graph_objects as go

    figure = go.Figure()
    figure.add_trace(go.Scatter(
        x=result["time_values"],
        y=result["left_knee_values"],
        mode="lines",
        name="왼쪽 무릎",
    ...
    figure.add_trace(go.Scatter(
        x=result["time_values"],
        y=result["right_knee_values"],
        mode="lines",
        name="오른쪽 무릎",
        line=dict(color="#F97316", width=3),
    ))
    figure.add_hline(
        y=110, line_dash="dot", line_color="#16A34A",
        annotation_text="충분히 앉은 기준 110°",
        annotation_position="bottom right",
    )
    figure.add_hline(
        y=155, line_dash="dot", line_color="#64748B",
        annotation_text="일어선 기준 155°",
...
def pose_points_for_chart(result):
    ...
    if result["lowest_pose_world"]:
        source = result["lowest_pose_world"]
        return [
            (item[0], item[2], -item[1], item[3])
            for item in source
        ]
    source = result["lowest_pose_normalized"]
    return [
...
def make_pose_3d(result):
    """가장 깊게 앉은 순간을 회전 가능한 3D 골격으로 보여 줍니다."""
    import plotly.graph_objects as go
...
<div class="result-grid">
  <div class="result-item"><span>전체 반복</span><strong>{result["reps"]}회</strong></div>
  <div class="result-item"><span>바른 반복</span><strong>{result["correct_reps"]}회</strong></div>
  <div class="result-item"><span>연습 점수</span><strong>{result["score"]}점</strong></div>
  <div class="result-item"><span>최저 무릎각</span><strong>{result["minimum_knee_angle"]:.0f}°</strong></div>
</div>
"""
...
<div class="pose-topic">휴대폰 영상으로 스쿼트 자세를 분석해요</div>
""", unsafe_allow_html=True)

    st.markdown(
        '<div class="guide">① 전신이 보이는 45초 이하 영상을 준비하세요.<br>'
        '② 촬영 방향을 선택하고 영상을 올리세요.<br>'
        '③ 분석 시작을 누르면 반복 횟수와 관절 각도를 알려 줍니다.</div>',
        unsafe_allow_html=True,
    )
    ...
    if "pose_analysis_result" not in st.session_state:
        st.session_state.pose_analysis_result = None
        st.session_state.pose_upload_digest = ""
        st.session_state.pose_analysis_view = ""
    ...
    if uploaded_video is not None:
        ...
        if analyze_button:
            ...
            status = st.empty()
            status.info("AI가 영상에서 관절을 찾고 있습니다. 잠시 기다려 주세요.")
        ...
        st.info("휴대폰으로 촬영한 스쿼트 영상을 먼저 올려 주세요.")
    ...
    if result:
        ...
        st.download_button(
            ...
            file_name="squat_pose_analysis.mp4",
            mime=result["video_mime"],
            width="stretch",
        )

        with st.expander("📈 시간에 따른 무릎 각도", expanded=True):
            st.plotly_chart(
        ...
        with st.expander("🧍 가장 깊게 앉은 순간을 3D로 보기", expanded=True):
            st.plotly_chart(
                make_pose_3d(result),
                width="stretch",
                config={
